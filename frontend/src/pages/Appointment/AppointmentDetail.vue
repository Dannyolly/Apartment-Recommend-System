...
onLoad(async (query)=>{
  ...
  state.value =  RoomBookingStateMap[order.value.state]
  ...
  infoList.value[3].right =  order.value.visitTime ? getFullTime( new Date(order.value.visitTime)) :''
  //console.log(order.value);
  recommendList.value = (await ServiceManager.RoomService.getRecommendRooms(order.value.houseId,5,1)).result 
})
...
<template>
  ...
  <div   class='container'>
    ...
    <div  class="state-bar">
      <div class="left">
        ...
        <div class="desc">
          ...
          <div class="subtitle">公寓{{ state }}</div>
        </div>
      </div>
      <div class="right">
    ...
    <div class="detail-container">
      <Detail  :state="state" :room="order.room"/>
    ...
    <div class="recommend-container">
        ...
        <House v-for="room in recommendList" :paddingRight="10" :post="room" />
    </div>
