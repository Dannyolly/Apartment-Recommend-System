...
import { SerivceOrderOption, ServiceOrder, ServiceOrderStateMap } from "@/types/serviceEntity/Service";
defineOptions({
  name: "ElementPlus"
})
...
const handleDelete = async (row:UserInfo) => {
  ...
  const { data } =  await ServiceManager.ClientOrderSerivce.deleteById(row.id)
  if(data){
...
const searchData = reactive<SerivceOrderOption>({})
const getParamsBySearchData = ()=>{
  let params:SerivceOrderOption = {}
...
const getTableData =async () => {
  const params = getParamsBySearchData()
  let dataTemp:ServiceOrder[] = []
  if(Object.values(params).length==0){
    const { data } = await ServiceManager.ClientOrderSerivce.select({
  ...
  dataTemp.forEach(v=>{
    v.serviceTime = getFullTime(new Date(v.serviceTime || ''))
    v.createTime = getFullTime(new Date(v.createTime || ''))
    //@ts-ignore
    v.state = ServiceOrderStateMap[Number(v.state)].title
  })
...
const getItemCount = async () =>{
  ...
  if(Object.values(params).length!==0){
    const { data } = await ServiceManager.ClientOrderSerivce.count(params)
    paginationData.total = data
    return
  }
  const { data } = await ServiceManager.ClientOrderSerivce.count()
  paginationData.total = data
}
onMounted(()=>{
  getItemCount()
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData">
          ...
          <el-table-column prop="curSer" label="所选服务" align="center" />
          <el-table-column prop="curPri" label="价格" align="center" />
          <el-table-column prop="createTime" label="订单创建时间" align="center" />
          <el-table-column prop="servicTime" label="预计服务时间" align="center" />
          <el-table-column prop="requirementDesc" label="需求"  align="center"/>
