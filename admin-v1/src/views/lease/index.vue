...
//#region 查
const tableData = ref<LeaseOrder[]>([])
const searchFormRef = ref<FormInstance | null>(null)
const searchData = reactive<LeaseOrderOption>({})
const getParamsBySearchData = ()=>{
  let params:LeaseOrderOption = {}
...
const getTableData =async () => {
  ...
  const { data } = await ServiceManager.LeaseOrderService.select({
      ...params,
  ...
  data.forEach(v=>{
    // @ts-ignore
    v.state =  Number(v.state) === 1?'待上门':'已完成'
...
const getItemCount = async () =>{
  ...
  const { data } = await ServiceManager.LeaseOrderService.count({
    ...params,
    page:paginationData.currentPage,
    pageSize:paginationData.pageSize
  })
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData">
          ...
          <el-table-column prop="houseType" label="客房類型" align="center" />
          <el-table-column prop="appointmentTime" label="预计上门时间" align="center" />
